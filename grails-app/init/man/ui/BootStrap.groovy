    ...
    def init = { servletContext ->
        User.withTransaction { status ->
            if (User.list().isEmpty()) {
                Date now = new Date()
                Organization org = new Organization(uuid: "ADSJDHAO123987asdkj", name: "CoVeritas", created: now, lastUpdated: now).save(failOnError: true)
                Role adminRole = new Role(name: "Admin").save(failOnError: true)
                Role userRole = new Role(name: "User").save(failOnError: true)
                User.create("asdjA12364SDUHADIh", "admin", org, "@dm1n", [adminRole] as Set<Role>)
            }
        }
    }
    def destroy = {
